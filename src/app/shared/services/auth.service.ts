...
export class AuthService {
    ...
    deleteAccount(): Observable<any> {
        ...
        return this.http.post(`${this.url}:delete?${this.keyQuery}`, token).pipe(
    ...
    login(credentials: UserCredentials): Observable<any> {
        ...
        return this.http.post(`${this.url}:signInWithPassword?${this.keyQuery}`, credentials)
            .pipe(
                tap(this.setToken),
                catchError(this.setError.bind(this))
            )
    ...
    signup(credentials: UserCredentials): Observable<any> {
        credentials.returnSecureToken = true
        return this.http.post(`${this.url}:signUp?${this.keyQuery}`, credentials)
            .pipe(
                tap(this.setToken),
                catchError(this.setError.bind(this))
            )
    ...
    updateEmailOrPassword(newValue: {email: string} | {password: string}): Observable<any> {
        ...
        return this.http.post(`${this.url}:update?${this.keyQuery}`, body).pipe(
            catchError(this.setError.bind(this)),
            tap(this.setToken)
        )
    }
    ...
    private setError(error: HttpErrorResponse) {
        switch(error.error.error.message) {
            ...
            case 'EMAIL_EXISTS':
                ...
                break
            case 'WEAK_PASSWORD':
                this.alert.warning('Password should contain at least 8 characters')
                break
